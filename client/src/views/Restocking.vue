    ...
    <div v-if="!loading && !error" class="card">
      ...
      <div v-else class="table-container">
        <table>
          ...
          <tbody>
            <tr v-for="item in allocatedItems" :key="item.item_sku">
              <td>{{ item.item_name }}</td>
              <td class="sku-cell">{{ item.item_sku }}</td>
              <td><span :class="['badge', item.trend]">{{ item.trend }}</span></td>
              <td class="text-right">{{ item.qty_to_restock.toLocaleString() }}</td>
...
export default {
  ...
  setup() {
    ...
    const budgetUsed = computed(() =>
      allocatedItems.value.reduce((sum, item) => sum + item.subtotal, 0)
    )
    ...
    const placeOrder = async () => {
      ...
      try {
        await api.createRestockingOrder({
          items: allocatedItems.value.map(item => ({
            sku: item.item_sku,
            name: item.item_name,
            quantity: item.qty_to_restock,
